"""
Comprehensive Unseen Data Evaluation with Scaled Sample Sizes (Novel Non-Tree Models).
...
3. 50-Iteration Monte Carlo Bootstrap (Over 12,000+ unseen evaluations)
"""
...
from src.models import find_optimal_feature_scaler, get_novel_non_tree_models


def run_large_unseen_evaluation():
    print("=" * 108)
    print("LARGE-SAMPLE UNSEEN DATA GENERALIZATION ASSESSMENT: DATASET 2 (NOVEL NON-TREE MODELS)")
    print("=" * 108)
    ...
    print("-" * 108)
    print(f"{'Model Name':<42} {'Unseen Accuracy':<18} {'Correct / Total':<18} {'Weighted F1':<14} {'Macro F1':<12}")
    print("-" * 108)

    models_30 = get_novel_non_tree_models(random_seed=42)
    exp1_records = {}
    ...
    for name, model in models_30.items():
        ...
        print(f"{name:<42} {acc*100:>10.2f}%      {correct}/{total:<13} {wf1:>10.4f}     {mf1:>8.4f}")
    ...
    models_40 = get_novel_non_tree_models(random_seed=42)
    exp2_records = {}
    ...
    # =========================================================================
    # EXPERIMENT 3: 50-ITERATION MONTE CARLO BOOTSTRAP (12,250+ Unseen Evaluations)
    # =========================================================================
    print(f"\n{'-' * 108}")
    print("[EXPERIMENT 3] 50-RUN MONTE CARLO SIMULATION (12,250+ TOTAL UNSEEN EVALUATIONS)")
    print(f"{'-' * 108}")
    print("Evaluating stability across 50 different randomized 70/30 train/unseen splits...")

    monte_carlo_splitter = StratifiedShuffleSplit(n_splits=50, test_size=0.30, random_state=42)
    ...
    print(f"{'Model Name':<42} {'Mean Unseen Acc (%)':<22} {'95% Confidence Interval':<28} {'Min - Max Range':<16}")
    print("-" * 108)
