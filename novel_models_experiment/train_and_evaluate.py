...
def run_full_experiment_and_generate_report():
    ...
    report_lines = [
        ...
        "1. **Student \"Landmark Personas\" (Multi-Prototypes):** Not all stressed students look the same. KMAC creates multiple landmark personas (sub-cluster prototypes) for each stress category instead of assuming one average profile fits everyone.",
        "2. **The \"Smart Distance Magnifier\" (Metric Learning):** Standard distance formulas treat every survey question equally. KMAC learns a dynamic multiplier ($\mathbf{w} = \\exp(\\boldsymbol{\\theta})$) via Adam optimization that magnifies crucial psychological signals (like anxiety and sleep) while ignoring background noise.",
        "3. **Dual-Kernel Soft Spotlight (Hybrid RBF-Laplacian Attention):** KMAC calculates how close the student is to every landmark using a combination of smooth spherical distance (RBF) and sharp diamond distance (Laplacian). A temperature dial ($\tau$) turns these distances into calibrated percentage probabilities across Low, Medium, and High stress.\n",
        "```mermaid",
        "flowchart TD",
        ...
        "#### Simple Intuitive Explanation:",
        "1. **Noise-Canceling Feature Gates (GLU):** Like noise-canceling headphones, each layer has two pathways: a *signal pathway* and a *gate pathway*. The gate calculates a multiplier between $0.0$ and $1.0$. If a survey feature is irrelevant, the gate closes and mutes the noise.",
        "2. **Channel Recalibration (Squeeze-and-Excitation):** Across the 128 hidden channels, the network checks which combinations of features (e.g. high workload combined with poor sleep) are most critical, dynamically boosting their signal strength.",
        "3. **Compass Direction over Distance (Hyperspherical Cosine Head):** Standard neural networks make decisions based on vector length (which can explode during training). RGFN maps both the student's hidden embedding $\mathbf{z}$ and the target class centers $\mathbf{w}_c$ onto the surface of a **unit sphere ($\\mathbb{S}^{d-1}$)**. It classifies the student based purely on **angular direction ($\cos \\theta$)**, ensuring rock-solid numerical stability.\n",
        "```mermaid",
        "flowchart TD",
